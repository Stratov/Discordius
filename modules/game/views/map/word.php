...
<div class="map_coordonne">
		...
		<?php endif; ?>
				...
				&nbsp;<img src="<?php echo url::base(); ?>images/orther/sound.png"  alt="Audio" id="control_sound" class="invis"/><br />
		...
		for( $y = 0; $y < $tailleMapY; $y++ )
		{
				...
				for( $x = 0; $x < $tailleMapX; $x++ )
				{
						...
						if( isset( $otherUsers[$x.'-'.$y] ) && ( $otherUser = $otherUsers[$x.'-'.$y]) ){
								...
								echo '<div id="otherUser-'.$otherUser->id.'" class="otherUsers" style="background-image:url(\''.url::base().'images/character/'.$otherUser->avatar.'\');  '.$positionDiv.'"><div class="usernameUsers" style="colors:'.$cadmin.';">'.$otherUser->username;
								if($Oguilde->inGuilde($otherUser->id)){echo '<br />['.$Oguilde->nomGuilde($otherUser->id).']';}
								echo '</div></div>'."\n";
						}
				}
		}
