<?php
 ...
 class Statistiques_Model extends Model {
	 ...
	 /**
		* Permet d'aller chercher des statistiques particulières
		*
		* @return object retourne la ligne avec certaines valeurs des stats
		*/
	 public function get_statistic( $statistic_name )
	 {
		 $query = $this->db->from( 'statistics' )->where(' name= "'.$statistic_name.'"')->limit( 1 )->get();
		 $array_query = $query->as_array();

		 return $query->count() ? $array_query[0]->value : FALSE;
	 }
	 ...
	 /**
		...
		* @return object retourne la ligne avec certaines valeurs des stats
		*/
	 public function update_statistic( $statistic_name, $statistic_value )
	 {
		 $sql = 'UPDATE statistics SET value="'.$statistic_value.'" WHERE name="'.$statistic_name.'"';

		 return $this->db->query( $sql ) == 1 ? TRUE:FALSE;
	 }
