<?php
 ...
 class User_Model extends Auth_User_Model {
     ...
     /**
      * Retourne la liste des utilisateurs connectés, trié par région.
      *
      * Un joueur est considéré connecté si sa dernière action remonte à moins de 5 minutes.
      *
      * Example:
      * [
      *   'region 1' => [ 'Alice', 'Bob' ],
      ...
      * ]
      */
     public function get_connected_users_by_region()
     {
         $time = time() - 300;
         $query = $this->db->from('regions', 'users')
             ->select('regions.name', 'users.username', "SUM(roles.name = 'admin') as is_admin")
             ->join('roles_users', 'roles_users.user_id', 'users.id', 'LEFT')
             ->join('roles', 'roles.id', 'roles_users.role_id', 'LEFT')
             ->where('users.last_action > '.$time)
         ...
         if ($query->count()) {
             foreach ($query as $row) {
                 ...
                 $result[$row->name][] = array($row->username, $row->is_admin);
             }
         }
         return $result;
     }
