...
$(function() {
		...
		window.onkeydown = function(e) {
				if(e.keyCode == 13 && !focusChat && $('.blockAction').is(':hidden') && $('#commandAction').is(":hidden") )
				{
                    console.log('!!');
						module();
                    console.log('!!2');
				...
				if (move_keys.indexOf(e.keyCode) == -1)
					move_keys.push(e.keyCode);
				if (move_keys.length == 1)
					startMoveAuto(e.keyCode);
		};
		
		window.onkeyup = function(e) {
			var index = move_keys.indexOf(e.keyCode);
			move_keys.splice(index,1);
...
{
		if (focusChat || $('#content_pwd').is(':visible') || focusInput)
				return;
    ...
    if (move_keys.length != 0) {
			setTimeout( function() {
				startMoveAuto(move_keys[move_keys.length-1]);
			}, 10 );
		}
}
...
console.log("stop");
		if(direction == 90
				...
				if( !enCours && !focusChat && $('.blockAction').is(':hidden') && $('#commandAction').is(':hidden'))
				{
						action(); 
						module();
				}
